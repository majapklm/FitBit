...
def complete(request):
    ...
    user_fitbit_datas = get_data(request, 'activities', 'steps')
    resource_type = TimeSeriesDataType.objects.get(category=1, resource='steps')
    for data in user_fitbit_datas:
        if TimeSeriesData.objects.filter(user=user, resource_type=resource_type, date=data['dateTime']).exists():
            series_data = TimeSeriesData.objects.get(user=user, resource_type=resource_type, date=data['dateTime'])
            series_data.value=data['value']
            series_data.save()
        else:
            series_data =  TimeSeriesData.objects.create(user=user, 
...
@require_GET
def get_existing_user_data(request):
    ...
    # Authorization ='Basic MjJCUVA1OmZiZDRlMGY1NDZjOTk1MjY3ZDIxZTQ4MjRiZDA3YmFi'
    # 'client_id':utils.get_setting('FITAPP_CONSUMER_KEY'),
    # 'client_secret' :utils.get_setting('FITAPP_CONSUMER_SECRET'),

    header = { 'Authorization':'Basic MjJCUVA1OmZiZDRlMGY1NDZjOTk1MjY3ZDIxZTQ4MjRiZDA3YmFi', 
    ...
    response = requests.post(fitbit_token_uri, data, headers=header)
    if response.status_code != 200:
        return redirect(reverse('fitbit-error'))
    fits = response.json()
    fitbit_user.access_token = fits['access_token']
    fitbit_user.refresh_token = fits['refresh_token']
    fitbit_user.save()
    ...
    next_url = utils.get_setting('FITAPP_LOGIN_REDIRECT')
    return redirect(next_url)
